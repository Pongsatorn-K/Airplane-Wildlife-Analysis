...
-   Insight
    ...
    Let compare incidents by gull and mourning dove.
```{r}
df_gd <- data.frame(read_csv("GD.csv", show_col_types = FALSE))
ggplot(data=df_gd, aes(x=Incident_Month, y=Count, color=Species_Name)) + 
  geom_line() + geom_point() + 
  geom_text(aes(label=Count), vjust=-0.5) + 
  labs(x="Month",y="Number of incidents", title="Incidents per month",
       subtitle="Incidents cause by Gull and Mourning Dove") + 
  scale_x_continuous(n.breaks=12)
```
Incidents by Gull is peak around 600 incidents in April and October while incidents by Mourning dove is peak at 722 incidents in August, which support migration hypothesis.
