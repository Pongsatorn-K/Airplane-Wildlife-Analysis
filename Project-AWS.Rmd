...
author: "Author: Pongsatorn Konggate"
...
This is a project for Google Data Analytics course by Coursera. Which will mostly use the steps of the data analysis process: ask, prepare, process, analyze, share, act for this analysis. Topic of analysis will focus on airplane and wildlifes collision which can cause damage to airplane and in some case cause injuries to passenger or worse, fatalities.

## Task to investigate

-   **Trend of airplane and wildlifes collision and how to reduce incidents.**

## Data

-   **Data Source**: This dataset is from kaggle's [Aircraft Wildlife Strikes, 1990-2015](https://www.kaggle.com/datasets/faa/wildlife-strikes) under [CC0 license](https://creativecommons.org/publicdomain/zero/1.0/). With the original data source from [FAA Wildlife Stirke Database](https://wildlife.faa.gov/home) which contains records of voluntary reported wildlife strikes in United States.
...
-   Insight
    ...
    Incident increasing by year that may come from increase in number of flights, increase in populations of the wildlifes in the nearby area.
...
ggplot(data=df_incident_m, aes(x=Incident_Month, y=Count)) + 
  ...
  labs(x="Month", y="Number of incidents", title="Incidents occur per month") + 
...
-   Insight
    ...
    Let compare incidents by top 5 known species.
```sql
SELECT Incident_Month, Species_Name, Count         
FROM
  (
  SELECT  aws.Incident_Month, aws.Species_Name, Count(*) as Count,
          aws_dmg.Count as Dmg,
          DENSE_RANK() OVER(ORDER BY aws_dmg.Count DESC) AS Dmg_Rank --> Ranking specie dmg
  FROM `aircraft-wildlife-strikes.aw_strikes.aircraft wildlife strikes` as aws
    INNER JOIN `aircraft-wildlife-strikes.aw_strikes.aws_dmg` as aws_dmg --> previous table
    ON aws.Species_Name = aws_dmg.Species_Name
  WHERE aws.Species_Name IS NOT NULL 
        AND aws.Species_Name NOT LIKE '%UNKNOWN%'   --> Filter out unknown bird
        AND aws.Aircraft_Type = 'A'
  GROUP BY aws.Incident_Month, aws.Species_Name, Dmg
  ORDER BY Dmg DESC
  )
WHERE Dmg_Rank < 6
ORDER BY Dmg_Rank ASC
```
```{r}
df_top_dmg <- data.frame(read_csv("top5_dmg_month.csv", show_col_types = FALSE))

ggplot(data=df_top_dmg, aes(x=Incident_Month, y=Count, color=Species_Name)) + 
  geom_line() + 
  #geom_text(aes(label=Count), vjust=-0.5, size=3) + 
  labs(x="Month",y="Number of incidents", title="Incidents per month",
       subtitle="Incidents cause Top 5 known species") + 
  scale_x_continuous(n.breaks=12)
```

Incidents by Gull is peak around 600 incidents in April and October while incidents by other species is rising during summer with it peak at August and incidents are dropping when enter autumn, which support migration hypothesis.
...
ggplot(data=df_ca,aes(x=Height)) +
  geom_histogram(data=subset(df_ca, Flight_Phase==("APPROACH")), binwidth=20) +
  xlim(-20,3000) +
  labs(x="Height (feet)", title="Histogram")
...
[We can manipulate data by define height of each phase and change Flight_Phase according to defined height but it does not seem to get us more information in this analysis.]{style="color:purple"}


Next let see distance between airport and location that incidents occur.
```{r}
df_dt <- data.frame(read_csv("Distance.csv", show_col_types = FALSE))

ggplot(data=df_dt, 
      aes(x=Distance)) +
  geom_histogram(binwidth=0.5, boundary=0) +
  xlim(0,10) +
  labs(x="Distance (Nuatical mile)" , 
      title="Histogram of distance",
      subtitle="bin=0.5")
```

Most incidents happen around airport.
...
-   Most incident occur during 'Approach' phase follow by 'Take off run', 'Landing roll' and 'climb phase'.
    -   Incident occur upon arrival is significant higher compare to departure ,further exploration about 'Approach' phase such as vision from airplane and bird perspective in order to reduce incident in this phase.
    
# Conclusion
- Since most of incidents occur around airport area, habitat management around airport might be an effective strategy to control population of wildlife around airport area. Bird migration further exploration should be conduct in order to handle each habitat of individual species effectively for each season.
